...
declare_id!("75P17oxakz6cSriC2eEFyBkudbmHGjiGX3DcCFbgcuAx");
...
pub struct CreateVestingAccount<'info> {
    #[account(mut)]
    pub signer: Signer<'info>, // the account paying for the creation of the vesting account
    ...
    pub vesting_account: Account<'info, VestingAccount>, // the vesting account to be created

    pub mint: InterfaceAccount<'info, Mint>, // the mint for the token account
    ...
    pub treasury_token_account: InterfaceAccount<'info, TokenAccount>, // the token account to hold the vested tokens

    pub system_program: Program<'info, System>, // because we create an account
    pub token_program: Interface<'info, TokenInterface>, // because we crate a token account
}
